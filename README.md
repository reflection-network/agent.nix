...
```nix
# Capsule switches from agent-nix to adapter-claude — same config, different backend
inputs.adapter-claude.url = "github:reflection-network/adapter-claude";
```

The adapter calls `agent-nix.lib.mkAgent` internally for validation, then builds a richer Docker image with the adapter's runtime (Claude Code CLI, Telegram transport, etc.).
